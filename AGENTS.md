...
- For `/shelf-movers/`, do not conflate tool/source changes with Lee's browser-local drag decisions. Before claiming curation decisions were applied, explicitly verify/export `localStorage.shelfMovers.v1` from Lee's active browser context or ask Lee for the export JSON; if Codex's automation context is empty, say that clearly and do not apply or summarize missing decisions.
